...
joint_full <- readRDS("../../run_mrp/data/acs.rds")
occscore_full <- readRDS("state_occscore.rds")
...
years <- sort(unique(joint_full$year))
acs_years <- system("ls ../../downloaded_data/acs/state_data_raw_*.rds", intern=TRUE)
acs_years <- as.numeric(gsub(".rds", "", gsub(".+state_data_raw_", "", acs_years)))

registerDoMC(30)
raw <- foreach (year = years) %dopar% {

  acs_year <- acs_years[which.min(abs(year - acs_years))]
  geog <- readRDS(paste0("../../downloaded_data/acs/state_data_raw_", acs_year, ".rds"))
  ...
  joint <- joint_full[joint_full$year == year,]
  occscore <- occscore_full[joint_full$year == year,]

  fips_from_state <- target$fips
  names(fips_from_state) <- target$state
  ...
  jpeg(paste0(year, "_check_delta_before.jpeg"), width=20, height=24, units="in", res=300)
  par(mfrow=c(9,9), mar=c(1,1,3,1), mgp=c(1,0.01,0), tck=0.005, lwd=0.5, pty="s", family="PT Sans")
  CheckDelta(dat=occscore)
  dev.off()

  jpeg(paste0(year, "_check_delta_after.jpeg"), width=20, height=24, units="in", res=300)
  par(mfrow=c(9,9), mar=c(1,1,3,1), mgp=c(1,0.01,0), tck=0.005, lwd=0.5, pty="s", family="PT Sans")
  CheckDelta(dat=out)
  dev.off()
...
# make sure the order lines up
if (!(all(sort(joint_full$year) == joint_full$year) & all(sort(years) == years)))
  stop("ordering wrong")
...
saveRDS(out, file="state_occshift.rds")
