...
target <- readRDS("../../adhoc/tract_smoothing/tract_smoothing.rds")

################################################################################################################
# load joint data

joint <- readRDS("tract_2018_joint.rds")
occscore <- readRDS("tract_2018_occscore.rds")
...
saveRDS(out, file="tract_2018_occshift.rds")
...
CheckDelta <- function(dat) {
  ...
  for (suffix in suffixs) {
    ...
    for (y in ys) {
      message("  ", y)
      plot(tmp[, paste0(y, ".x")], tmp[, paste0(y, ".y")], pch=20, cex=0.25, 
